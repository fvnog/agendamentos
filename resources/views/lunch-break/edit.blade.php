<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            <i class="fas fa-edit"></i> Editar Horário de Almoço
        </h2>
    </x-slot>
        ...
        <div class="max-w mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-900 shadow-lg rounded-lg p-6 text-white">
                <h3 class="text-lg font-semibold mb-6">
                    <i class="fas fa-utensils"></i> Ajuste o horário do seu almoço
                </h3>
                ...
                <form method="POST" action="{{ route('lunch-break.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="time" name="start_time" id="start_time" 
                               ...
                               value="{{ old('start_time', \Carbon\Carbon::parse($lunchBreak->start_time)->format('H:i')) }}" required>
                        @error('start_time')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="time" name="end_time" id="end_time" 
                               ...
                               value="{{ old('end_time', \Carbon\Carbon::parse($lunchBreak->end_time)->format('H:i')) }}" required>
                        @error('end_time')
                            <div class="text-sm text-red-500 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <!-- Botão de Atualizar -->
                    <button type="submit" 
                            class="w-full py-3 bg-yellow-500 text-white font-semibold rounded-lg hover:bg-yellow-600 transition duration-300 flex items-center justify-center gap-2">
                        <i class="fas fa-save"></i> Atualizar Horário de Almoço
                    </button>
                </form>
            </div>
        </div>
